...
if (isAuth !== "usuario") {
  ...
  const titulo = document.querySelector(".carrito_titulo");
  ...
  if (carrito.length !== 0) {
    ...
    window.addEventListener("click", (evento) => {
      if (evento.composedPath()[0].classList[0] === "eliminar_producto") {
        ...
        let precioProducto =
          elemento.querySelector(".precio_producto").innerHTML;
  ...
  boton.addEventListener("click", () => {
    ...
    usuarioServicios.detalleUsuario(userId).then((response) => {
      if (
        response.domicilio === "" ||
        response.ciudad === "" ||
        response.cp === "" ||
      ...
      if (error.style.display === "none") {
        usuarioServicios
          .actualizarCarritoUsuario(userId, [])
          .then((resp) => {
            console.log(resp);
          })
          .finally(() => {
            window.location.href = "../screens/compra.html";
          });
      }
    });
  });
